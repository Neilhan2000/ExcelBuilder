...
class DataMapper:
    ...
    def init_multiple_merged_cell_item(
        ...
        for i in range(convert_position_to_col_number(find_start_position(split_position[0])),
                       convert_position_to_col_number(find_end_position(split_position[-1])) + 1):
            work_sheet[convert_col_and_row_to_position(col_number=i, row_number=border_row)].border = border
    ...
    def init_row_and_column_merged_item(
        ...
        def traverse_all_rows_and_set_border(col_list: list, row_list: list, col_num, row_num):
            if row_num <= row_list[-1]:
                ...
                traverse_all_rows_and_set_border(col_list=col_list, row_list=row_list, col_num=col_num,
                                                 row_num=row_num + 1)
